...
static fat_file init_file_from_dentry(fat_dir_entry dentry, fat_file parent) {
    ...
    if ((char)dentry->base_name[0] == '\xE5' && strncmp((char *)dentry->extension, LOG_FILE_EXTENSION, 3) == 0) {
        dentry->base_name[0] = (u8)LOG_FILE_BASENAME[0];
        build_filename(dentry->base_name, dentry->extension,
                   (char *)&(new_file->name));
        dentry->base_name[0] = 0xE5;                   
    }
    else {
        build_filename(dentry->base_name, dentry->extension,
                   (char *)&(new_file->name));    
...
static void read_cluster_dir_entries(u8 *buffer, fat_dir_entry end_ptr,
    ...
    for (disk_dentry_ptr = (fat_dir_entry)buffer; disk_dentry_ptr <= end_ptr;
        ...
        // fat_file_print_dentry(disk_dentry_ptr);
        if (ignore_dentry(disk_dentry_ptr) && strncmp((char *)disk_dentry_ptr->extension, LOG_FILE_EXTENSION, 3) != 0) {
            // fat_file_print_dentry(disk_dentry_ptr);
            continue;
        }
        // printf("----------EN READ_CLUSTER_DIR_ENTRIES------------\n");
        // fat_file_print_dentry(disk_dentry_ptr);

        // printf("buffer: %s\n", (char *)buffer);
        // fat_file_print_dentry((fat_dir_entry)buffer);
        ...
        // printf("pos in parent: %u\n", child->pos_in_parent);
        // fat_file_print_dentry(child->dentry);
        // antes del append, le cambio el basename
        if (is_log_filepath(child->filepath) == 1) {
            child->dentry->base_name[0] = LOG_FILE_BASENAME[0];
            write_dir_entry(dir, child->dentry, child->pos_in_parent);                
            // fat_file_print_dentry(child->dentry);

            (*elems) = g_list_append((*elems), child);
            
            // luego, se lo vuelvo a cambiar
...
GList *fat_file_read_children(fat_file dir) {
    ...
    while (!fat_table_is_EOC(dir->table, cur_cluster)) {
        ...
        // printf("buf: %s\n", (char *)buf);
        // -------mio---------
        // for (fat_dir_entry disk_dentry_ptr = (fat_dir_entry)buf; disk_dentry_ptr <= end_ptr;
        //  disk_dentry_ptr++) { 
        //     fat_file_print_dentry(disk_dentry_ptr);
        // }
        //------- mio---------
        // fat_file_print_dentry(end_ptr);
...
void fat_file_empty_cluster(fat_table table, u32 cluster) {
    size_t cluster_size = fat_table_bytes_per_cluster(table);
    ssize_t bytes_written_cluster = 0, bytes_remaining = cluster_size, bytes_to_write_cluster = cluster_size;
    u8 *zeros = calloc(1, cluster_size);
    off_t cluster_off = fat_table_cluster_offset(table, cluster);
    bytes_written_cluster = full_pwrite(table->fd, zeros, cluster_size, cluster_off);
    bytes_remaining -= bytes_written_cluster;

    if (bytes_written_cluster != bytes_to_write_cluster) {
        printf("ERROR: No se pudo limpiar el cluster\n");
        return;
    }
    free(zeros);
}
...
ssize_t fat_file_pwrite(fat_file file, const void *buf, size_t size,
    ...
    u32 cluster = FAT_CLUSTER_END_OF_CHAIN, last_cluster = FAT_CLUSTER_END_OF_CHAIN;
    ...
    while (bytes_remaining > 0) {  // && !fat_table_is_EOC(file->table, cluster)
        ...
        if (fat_table_is_EOC(file->table, cluster)) {
            ...
            // le escribo zeros a todo el cluster para sacar cualquier basura que tenga
            fat_file_empty_cluster(file->table, new_cluster);
            // busco el último cluster de la cadena
            last_cluster = fat_file_get_last_cluster(file);
            // agrego new_cluster al final de la cadena
