...
public class NameEntityProcessor {
    public static HashMap<Category, List<NamedEntity>> extractNamedEntitiesByCategory(String text,
        ...
        for (String candidate : candidates) {
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                if (keywords.contains(candidate)) {
                    ...
                    // categorizedEntities.computeIfAbsent(category, k -> new ArrayList<>()).add(ne);
            ...
            if (!found) {
                ...
                // categorizedEntities.computeIfAbsent(Category.OTHER, k -> new ArrayList<>()).add(ne);
                categorizedEntities.computeIfAbsent(Category.OTHER, k -> new ArrayList<>()).add(namedEntity);
            }
        }
        return categorizedEntities;
    }

    public static HashMap<Topic, List<NamedEntity>> extractNamedEntitiesByTopic(String text, List<String> candidates)
            throws IOException {
